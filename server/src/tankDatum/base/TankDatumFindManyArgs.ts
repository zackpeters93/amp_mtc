/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TankDatumWhereInput } from "./TankDatumWhereInput";
import { Type } from "class-transformer";
import { TankDatumOrderByInput } from "./TankDatumOrderByInput";

@ArgsType()
class TankDatumFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => TankDatumWhereInput,
  })
  @Field(() => TankDatumWhereInput, { nullable: true })
  @Type(() => TankDatumWhereInput)
  where?: TankDatumWhereInput;

  @ApiProperty({
    required: false,
    type: [TankDatumOrderByInput],
  })
  @Field(() => [TankDatumOrderByInput], { nullable: true })
  @Type(() => TankDatumOrderByInput)
  orderBy?: Array<TankDatumOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { TankDatumFindManyArgs };
